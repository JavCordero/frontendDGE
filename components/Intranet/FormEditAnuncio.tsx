...
import EditAnuncio from "../../hooks/useEditAnuncio";
import getAnuncioId from "../../hooks/useGetAnuncioId";
const SunEditor = dynamic(() => import("suneditor-react"), {
  ssr: false,
});

export const FormEditAnuncio = ({ idUser, anuncioId }) => {
  ...
  const editarAnuncio = async (e) => {
    e.preventDefault();
    const nuevo = await EditAnuncio(anuncioId, data, descripcion, areaInput);
    ...
    if (nuevo.mensaje) {
      Swal.fire("Anuncio editado con exito", "", "success").then((result) => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const isLoaded = await getAnuncioId(anuncioId);
      console.log(isLoaded);
      setDescripcion(isLoaded.descripcion);
      setData(isLoaded.texto);
      if (isLoaded.area) {
  ...
  return (
    <div>
      ...
      <hr />
      <form onSubmit={(e) => editarAnuncio(e)}>
        {step + 1 === 1 ? (
          <>
